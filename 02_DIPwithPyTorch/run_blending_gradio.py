...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    grad_x = torch.tensor([[-1., 0., 1.], [-2., 0., 2.], [-1., 0., 1.]])
    grad_y = torch.tensor([[-1., -2., -1.], [0.,  0.,  0.], [1.,  2.,  1.]])

    grad_x = grad_x.view(1, 1, 3, 3).expand(3, 3, -1, -1).to(device)
    grad_y = grad_y.view(1, 1, 3, 3).expand(3, 3, -1, -1).to(device)

    fg_grad_x = F.conv2d(foreground_img, grad_x, padding=1)[foreground_mask.bool().expand(-1, 3, -1, -1)]
    fg_grad_y = F.conv2d(foreground_img, grad_y, padding=1)[foreground_mask.bool().expand(-1, 3, -1, -1)]
    blend_grad_x = F.conv2d(blended_img, grad_x, padding=1)[background_mask.bool().expand(-1, 3, -1, -1)]
    blend_grad_y = F.conv2d(blended_img, grad_y, padding=1)[background_mask.bool().expand(-1, 3, -1, -1)]

    loss = torch.sum((fg_grad_x - blend_grad_x) ** 2 + (fg_grad_y - blend_grad_y) ** 2)
