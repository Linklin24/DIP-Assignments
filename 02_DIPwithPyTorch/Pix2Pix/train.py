...
def train_one_epoch(model, dataloader, optimizer, criterion, device, epoch, num_epochs):
    ...
    for i, (image_rgb, image_semantic) in enumerate(dataloader):
        ...
        if epoch % 5 == 0 and i == 0:
            save_images(image_semantic, image_rgb, outputs, f'train_results/{dataset}', epoch)
...
def validate(model, dataloader, criterion, device, epoch, num_epochs):
    ...
    with torch.no_grad():
        for i, (image_rgb, image_semantic) in enumerate(dataloader):
            ...
            # Save sample images every 5 epochs
            if epoch % 5 == 0 and i == 0:
                save_images(image_semantic, image_rgb, outputs, f'val_results/{dataset}', epoch)
...
def main(dataset):
    ...
    train_dataset = CustomDataset(name=dataset, list_file=f'train_list/{dataset}_train_list.txt')
    val_dataset = CustomDataset(name=dataset, list_file=f'val_list/{dataset}_val_list.txt')
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 20 == 0:
            ...
            torch.save(model.state_dict(), f'checkpoints/{dataset}/pix2pix_model_epoch_{epoch + 1}.pth')
